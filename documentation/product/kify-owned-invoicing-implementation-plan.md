...
| K7 Migration verification and controlled rollout | active | 2026-07-21: `db:check-raw` passes. K7 runtime proof is blocked before database access because `DATABASE_URL` is absent; platform readiness also reports missing app URL, cron secret, and SMTP configuration. Test/live autonomy reports cannot run without those prerequisites and a configured pilot tenant. | Pending external runtime prerequisites |
