...
| K6 Delivery, readiness, UX, and legacy regression | active | 2026-07-21: Kify delivery bypasses legacy provider resolution and attaches only private stored artifact bytes; new first-payment claims require a paid first payment. Focused delivery/claim tests, full node suite (423 tests), typecheck, lint (four pre-existing warnings), build, and diff check pass. | Pending K6 delivery-safety commit |
