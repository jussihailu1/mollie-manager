...
| K6 Delivery, readiness, UX, and legacy regression | active | 2026-07-21: Kify delivery bypasses legacy provider resolution and attaches only private stored artifact bytes; new first-payment claims require a paid first payment. Customer history and resend now carry canonical Kify IDs/numbers; the tenant-fenced document service serves private Kify streams and redirects legacy documents. Focused delivery/document/customer tests, full node suite (424 tests), typecheck, lint (four pre-existing warnings), build, and diff check pass. | `76f9650` + pending K6 document-consumer commit |
