...
| K5 Automated workflow integration | active | 2026-07-21: Kify is a first-class provider value but fails closed from the legacy adapter resolver; real-installment payment-link creation checks explicit tenant/customer Kify profile readiness before any side effect. Focused workflow tests, full node suite, typecheck, lint, and diff check pass. | Pending K5 readiness-guard commit |
