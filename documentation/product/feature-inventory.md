...
- [ ] Shared multi-tenant pilot foundation
  Status: in progress. Tenant, platform-operator, and operator-membership tables now exist. Tenant-owned subscription-policy defaults and tenant billing/accounting defaults now persist by `tenant_id` instead of one shared `"default"` row, and a manual `npm run tenant:provision` bootstrap path now creates tenant rows plus default settings. The migration also backfills one `legacy-default` tenant to preserve the current single-context deployment while the broader foundation is still being built. Remaining required work is membership-based auth/session resolution, tenant-owned Mollie and e-Boekhouden credentials, tenant-scoped core business tables, tenant-aware webhook/replay/repair/cron flows, and focused cross-tenant isolation tests. This slice remains intentionally limited to manual tenant provisioning. It still does not include self-serve signup, invites, platform billing, broad tenant administration UX, or tenant-specific SMTP.
