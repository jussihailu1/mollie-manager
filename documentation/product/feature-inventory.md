...
- [ ] Failed payment correctness
  Status: top priority. Classification foundation is implemented in pure helpers for failed, reversed, charged-back, mandate-problem, first-payment, mandate-only, and unsafe long-pending payment flows. Policy-safe customer notification composition is isolated in a pure helper but not wired to sending yet. The normal Needs Attention query now surfaces classified recurring failed/reversed/mandate-problem payments with safe manual next-action wording. Next slice should add dependency-injected notification orchestration. Actual pause, cancellation, dunning, fees, or collection consequences stay manual or future policy-controlled.
