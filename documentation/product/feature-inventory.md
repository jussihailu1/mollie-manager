...
- [ ] Needs attention dashboard
  Status: started. The dashboard and notifications page now use a shared Needs Attention query with stable item types, severity, entity links, and recommended actions for failed/reversed/expired payments, mandate problems, subscription payment-action/out-of-sync states, and failed webhooks. Raw webhook payloads stay out of normal operator output. Remaining sources to add: failed invoices, failed invoice email delivery, missing e-Boekhouden relation links, stale Mollie sync state, and setup blockers.
