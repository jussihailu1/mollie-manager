...
| Kify-owned invoicing | partial | K1-K6 exist: canonical tenant/customer billing data and exact VAT-inclusive cents validation; native PDFKit rendering; deterministic private Blob artifacts; automated first-payment and recurring issuance with frozen retry; private server-side email attachment; tenant-fenced history, resend, download, payment detail, readiness, and future-only profile editing. Controlled rollout remains pending. See `kify-owned-invoicing-implementation-plan.md`. |
