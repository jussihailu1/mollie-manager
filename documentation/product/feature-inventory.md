...
| Kify-owned invoicing | partial | K1-K5 exist: canonical tenant/customer billing data and exact VAT-inclusive cents validation; native PDFKit rendering; deterministic private Blob artifacts; automated first-payment and recurring issuance with frozen retry; private server-side email attachment. K6 delivery bypasses legacy-provider resolution for Kify and restricts new first-payment claims to paid first payments. Tenant UX/cutover and controlled rollout remain pending. See `kify-owned-invoicing-implementation-plan.md`. |
