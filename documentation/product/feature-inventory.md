...
| Kify-owned invoicing | partial | K1-K5 exist: canonical tenant/customer billing data and exact VAT-inclusive cents validation; native PDFKit rendering; deterministic private Blob artifacts; automated first-payment and recurring issuance with frozen retry; private server-side email attachment. Tenant UX/cutover and controlled rollout remain pending. See `kify-owned-invoicing-implementation-plan.md`. |
