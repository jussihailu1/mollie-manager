...
- [ ] Refactor oversized billing/orchestration modules into smaller policy, state-transition, and integration units
  Status: started. Extracted shared invoice creation and delivery retry batch helpers, shared invoice flow helpers, invoice creation metadata, invoice retry metadata, invoice retry candidate filtering, payment sync persistence, subscription sync persistence, customer billing repair, webhook ingestion processing, consent acceptance helpers, customer relation field mapping, customer archive policy, onboarding redirect path helpers, first-payment duplicate-blocker policy, first-payment plan policy, first-payment link record mapping, first-payment onboarding record mapping, first-payment invoice eligibility policy, first-payment invoice date resolution, first-payment invoice delivery payload mapping, payment-link sync record mapping, payment sync classification/metadata mapping, Mollie mode selection, Mollie resource lookup, subscription sync record mapping, and sync alert handling; the largest remaining modules still need further reduction.
