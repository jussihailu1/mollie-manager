...
- [ ] Refactor oversized billing/orchestration modules into smaller policy, state-transition, and integration units
  Status: started. Extracted shared invoice creation and delivery retry batch helpers, shared invoice flow helpers, webhook ingestion processing, consent acceptance helpers, first-payment plan policy, first-payment link record mapping, payment-link sync record mapping, payment sync classification/metadata mapping, and sync alert handling; the largest remaining modules still need further reduction.
