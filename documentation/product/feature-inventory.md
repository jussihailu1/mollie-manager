...
- [ ] Shared multi-tenant pilot foundation
  Status: now required before broader product release work can continue autonomously toward the intended target. The foundation must introduce tenant entity and membership modeling, explicit current-tenant session resolution while preserving normal URLs such as `/customers` and `/payments`, tenant-scoped business data, tenant-owned Mollie and e-Boekhouden credentials, tenant-owned policy/billing defaults, tenant-aware webhook/replay/repair/cron flows, and focused cross-tenant isolation tests. This slice is intentionally limited to manual tenant provisioning. It does not include self-serve signup, invites, platform billing, broad tenant administration UX, or tenant-specific SMTP.
