...
- [ ] Add integration coverage for consent, webhook sync, first-payment invoice creation, recurring invoice delivery retry, and repair flows
  Status: flow coverage now exists for webhook ingestion/status handling, the invoice creation batch helper, the invoice delivery retry batch helper, and the targeted repair surface; consent still needs deeper executable tests.
