...
describe("Kify invoice readiness", () => {
  ...
  it("checks tenant Kify setup without requiring a customer profile", () => {
    assert.deepEqual(describeKifyTenantInvoiceReadiness({ tenantProfile }), { ok: true });
    assert.equal(describeKifyTenantInvoiceReadiness({ tenantProfile: null }).ok, false);
  });
});
