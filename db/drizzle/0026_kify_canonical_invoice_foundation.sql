...
ALTER TABLE invoices
  ADD COLUMN IF NOT EXISTS canonical_invoice_number text,
  ADD COLUMN IF NOT EXISTS canonical_status canonical_invoice_status,
  ...
  ADD COLUMN IF NOT EXISTS canonical_snapshot_sha256 text,
  ADD COLUMN IF NOT EXISTS void_reason text;
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS invoices_tenant_mode_canonical_number_key
  ON invoices (tenant_id, mode, canonical_invoice_number)
  WHERE canonical_invoice_number IS NOT NULL;
--> statement-breakpoint
DO $$ BEGIN
  ALTER TABLE invoices ADD CONSTRAINT invoices_kify_canonical_snapshot_check
    CHECK (provider::text <> 'kify' OR (canonical_invoice_number IS NOT NULL AND canonical_status IS NOT NULL AND canonical_snapshot IS NOT NULL AND canonical_snapshot_sha256 IS NOT NULL));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS invoice_lines (
  ...
  CONSTRAINT invoice_lines_positive_quantity_check CHECK (quantity > 0),
  CONSTRAINT invoice_lines_positive_gross_check CHECK (gross_cents > 0),
  CONSTRAINT invoice_lines_vat_sum_check CHECK (net_cents + vat_cents = gross_cents)
);
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS invoice_render_attempts (
  ...
  status invoice_render_attempt_status NOT NULL,
  safe_error_code text,
  artifact_id text REFERENCES invoice_artifacts(id) ON DELETE RESTRICT,
  snapshot_sha256 text NOT NULL,
