DO $$ BEGIN
  CREATE TYPE "public"."payment_invoice_state" AS ENUM (
    'not_applicable',
    'pending_invoice',
    'invoice_creating',
    'invoice_created',
...
ALTER TABLE "payments"
  ADD COLUMN IF NOT EXISTS "invoice_state" "public"."payment_invoice_state" DEFAULT 'not_applicable' NOT NULL,
  ...
  ADD COLUMN IF NOT EXISTS "invoice_failed_at" timestamp with time zone;

CREATE INDEX IF NOT EXISTS "payments_invoice_state_idx"
  ON "payments" USING btree ("mode","payment_type","invoice_state");
...
normalized_targets AS (
  SELECT
    ...
    CASE
      ...
      THEN 'skipped'::"public"."payment_invoice_state"
      WHEN cm.match_count = 1
        AND cm.has_accepted_consent = 1
        AND cm.min_first_payment_mode = cm.max_first_payment_mode
        AND cm.max_first_payment_mode = 'real_installment'
        AND p.mollie_status = 'paid'
      THEN 'pending_invoice'::"public"."payment_invoice_state"
      ELSE 'not_applicable'::"public"."payment_invoice_state"
